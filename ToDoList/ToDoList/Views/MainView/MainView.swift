    ...
    var body: some View {
        ...
        NavigationStack {
                                ...
                                HStack{
                                    VStack(alignment: .leading) {
                                        Text(todoItem.importance != .common ? "\(todoItem.importance.rawValue) \(todoItem.text)" : todoItem.text)
                                            ...
                                            .background(
                                                NavigationLink("", destination: TaskView(todoItem: todoItem, showDate: todoItem.deadline != nil))
                                                    .opacity(0)
                                            )
                                        if let deadline = todoItem.deadline {
                                            HStack {
                                                Image(systemName: "calendar")
                                                Text(FileCache.formatDate(date: deadline, dateFormat: "d MMMM") ?? "")
                                            }
                                            .font(.caption)
                                            .foregroundColor(.gray)
                                        }
            ...
            .fullScreenCover(isPresented: $showCalendar) {
                CalendarView(fileCache: fileCache)
            }
            .popover(isPresented: $showTaskView) {
                TaskView(todoItem: TodoItem(text: "", importance: .common, deadline: nil, completed: false, creationDate: Date.now, editDate: Date.now), showDate: false)
            }
